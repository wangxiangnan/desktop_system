...
class _SvgEditorState extends State<_SvgEditor> {
  ...
  List<SvgLayer> get _layers => widget.canvas.layers;
  SvgLayer? get _selectedLayer => _selectedLayerId != null
      ? _layers.firstWhere(
          (l) => l.id == _selectedLayerId,
          orElse: () => _layers.isNotEmpty
              ? _layers.first
              : const SvgLayer(id: '', name: 'Default'),
        )
  ...
  void _duplicateLayer(String layerId) {
    ...
    final duplicatedLayer = SvgLayer(
      id: DateTime.now().millisecondsSinceEpoch.toString(),
  ...
  void _reorderLayers(int oldIndex, int newIndex) {
    ...
    final updatedLayers = List<SvgLayer>.from(_layers);
...
class _PropertiesPanel extends StatefulWidget {
  final SvgElement element;
  final Function(SvgElement) onUpdate;
  final VoidCallback onDelete;
